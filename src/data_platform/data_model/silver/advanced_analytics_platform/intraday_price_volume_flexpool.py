...
intraday_price_volume_flexpool_table = StaticTableModel(
    identifier=TableIdentifier(catalog="silver", schema="advanced_analytics_platform", name="intraday_price_volume_flexpool"),
    comment="Ordered and settled trade volumes and prices in the intraday market per asset group (project) within the flexpool in France. The trades reflect trades achievable with assets within the flexpool in France. The trades capture intertemporal price spreads in the intraday market or price spreads between intraday and day-ahead market.",
    schema=StructType([
        ...
        standard_columns.DataSourceColumn.to_struct_field(nullable=False),
        standard_columns.DataSystemColumn.to_struct_field(nullable=False),
        StructField("time_file_creation", TimestampType(), True, {"comment": "The datetime when the file is created in UTC"}),
        StructField("updated_at_source", TimestampType(), True, {"comment": "The datetime when the data are updated in UTC"})
    ]),
    sources=[TableIdentifier(catalog="bronze", schema="advanced_analytics_shared", name="too_timeseries_latest"), TableIdentifier(catalog="bronze", schema="advanced_analytics_shared", name="attribute")],
    tags={
