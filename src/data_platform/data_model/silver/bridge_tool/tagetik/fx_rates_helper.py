...
tagetik_fx_rates_helper_model = StaticTableModel(
    identifier=TableIdentifier(catalog="silver", schema="bridge_tool", name="tagetik_fact_fx_rates_helper"),
    schema=StructType([
        ...
        StructField(name = "yyyymm", dataType=StringType()),
        StructField(name = "date", dataType=StringType()),
        StructField(name = "average_rate", dataType=DoubleType()),
        StructField(name = "closing_rate", dataType=DoubleType()),
        standard_columns.LicenseColumn.to_struct_field()
    ]),
    comment="Tagetik Fx rates helper coverts all values to EUR based.",
    license="BT_DUMMY_TEST",
    primary_keys=["currency_to_code", "scenario_code", "period_code"],
        tags={
