...
price_dayahead_forecast_table = StaticTableModel(
    identifier=TableIdentifier(catalog="silver", schema="volue", name="price_dayahead_forecast"),
    schema=StructType([
        volue_standard_columns.CurveIdColumn.to_struct_field(),
        volue_standard_columns.CurveNameColumn.to_struct_field(),
        StructField("reference_date", TimestampType(), True, {"comment": "The date and time when the forecast was originally published"}),
        ...
        StructField("value", DoubleType(), True, {"comment": "The forecasted day-ahead power price, in the specified unit"}),
    ...
    comment="The forecasted day-ahead-power prices. Instance curves and tagged curves store multiple time series. An instance is identified with an issue date, holding the values for the particular forecast at the issue date. Tagged curves  normally store ensemble data and each time series in  a tagged curve is identified by a tag.",
    sources=[
