...
power_transfer_distribution_factor_exchange_forecast_table = StaticTableModel(
    identifier=TableIdentifier(
        catalog="silver",
        schema="volue",
        name="power_transfer_distribution_factor_exchange_forecast"
    ),
    comment=(
        "Power transfer distribution factor exchange forecast from Volue between various market areas. An instance is identified with an issue date, "
        "holding the values for the particular forecast at the issue date. Tagged curves normally store ensemble data and each time "
        "series in a tagged curve is identified by a tag."
    ),
    schema=StructType([
        ...
        volue_standard_columns.ReferenceDateColumn.to_struct_field(nullable=False),
        ...
        volue_standard_columns.TagColumn.to_struct_field(nullable=False),
        StructField("value", DoubleType(), True, {"comment": "The power transfer distribution factor exchange forecast in the specified unit"}),
