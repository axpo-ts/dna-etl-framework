...
write_table_name = f"{catalog_name}.{schema_name}.table_count"
...
create_ddl = f"""CREATE TABLE IF NOT EXISTS {write_table_name} (
  ...
  updated_by STRING,
  CONSTRAINT `pk_table_count` PRIMARY KEY (`timestamp`, `table_catalog`))
USING delta;"""

spark.sql(create_ddl)

read_tables = {
    "table_count_gold": "dna_prod_gold",
    "table_count_silver": "dna_prod_silver",
    "table_count_bronze": "dna_prod_bronze",
}
