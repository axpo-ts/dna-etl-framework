...
write_table_name = f"{catalog_name}.{schema_name}.source_system"
...
create_ddl = f"""CREATE TABLE IF NOT EXISTS {write_table_name} (
  ...
  updated_by STRING,
  CONSTRAINT `pk_source_system` PRIMARY KEY (`timestamp`, `catalog_name`))
USING delta;"""

spark.sql(create_ddl)

read_tables = {"sources_table_gold": "dna_prod_gold", "sources_table_silver": "dna_prod_silver"}
