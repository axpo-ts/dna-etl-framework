...
df_bronze = spark.sql(f"SELECT count(*) AS total FROM {table_name} WHERE mdoid IN (232000253, 232000254)")
total_count = df_bronze.collect()[0]["total"]
print("Total count: " + str(total_count))
if total_count > 0:
    sql_stmt = f"DELETE FROM {table_name} WHERE mdoid IN (232000253, 232000254)"
    print(sql_stmt)
    spark.sql(sql_stmt)
