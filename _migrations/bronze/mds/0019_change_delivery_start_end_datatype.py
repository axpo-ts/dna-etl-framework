...
tables = ["baltex_curve_price_index", "argus_curve_price_index"]
...
def get_table_ddl_script(catalog: str, schema: str, target_table: str) -> str:
    ...
    for metadata in schema_json["fields"]:
        if "table_id" not in metadata["name"] and metadata["name"] != "Volume":
            ...
            if metadata["name"] in ("DeliveryStart", "DeliveryEnd", "ReferenceTime"):
...
for table in tables:
    ...
    if spark.catalog.tableExists(full_table):
        ...
        delivery_start_column = next((col for col in columns if col.name == "DeliveryStart"), None)
        delivery_end_column = next((col for col in columns if col.name == "DeliveryEnd"), None)
        if (delivery_start_column and delivery_start_column.dataType != "timestamp") or (
            delivery_end_column and delivery_end_column.dataType != "timestamp"
        ):
            ...
            (
                ...
                .withColumn("DeliveryStart", f.to_timestamp(f.col("DeliveryStart")))
                .withColumn("DeliveryEnd", f.to_timestamp(f.col("DeliveryEnd")))
