...
schema = "entsoe"
tables_map_code = [
    "cost_congestion_management",
    "consumption",
    "consumption_forecast",
    "production",
    "production_dayahead_forecast",
    "production_forecast",
    "hydro_reservoir_level",
    "bid_balancing_energy",
    "procured_volume_price_balancing_capacity_legacy",
    "price_balancing_energy",
    "offered_volume_price_balancing_capacity",
    "bid_balancing_energy_aggregated",
    "price_energy_dayahead",
    "flow_based_allocation_parameter",
    "position_net_implicit_allocation",
]

for table in tables_map_code:
    ...
    spark.sql(
        ...
        RENAME COLUMN map_code TO area_map_code;"""
    )
...
for table in tables_in_out_map_code:
    ...
    spark.sql(
        f"""ALTER TABLE {table_name}
        RENAME COLUMN in_map_code TO in_map_area_code;"""
    )

    spark.sql(
        f"""ALTER TABLE {table_name}
        RENAME COLUMN out_map_code TO out_map_area_code;"""
    )
