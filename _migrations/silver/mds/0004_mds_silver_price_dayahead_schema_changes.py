...
def get_table_ddl_script(catalog: str, schema: str, target_table: str) -> str:
    ...
    return ", ".join(result) + ", " + table_id_prefix_clause + primary_key_str
...
for table in tables:
    ...
    if spark.catalog.tableExists(full_table):
        ...
        if any(col.name == "volume" for col in columns):
...
drop_tables = ["argus_curve_price_index", "baltex_curve_price_index"]
for table in drop_tables:
    full_table = f"{catalog_prefix}silver.{schema_prefix}{schema}.{table}"
    if spark.catalog.tableExists(full_table):
        print(f"Dropping table {full_table}")
