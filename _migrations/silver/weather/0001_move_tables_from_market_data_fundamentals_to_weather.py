...
table_mapping = {
    "forecast_meteomatics": "meteomatics_forecast",
    "actual_meteomatics": "meteomatics_actual",
    "precipitation_forecast_volue": "volue_precipitation_forecast",
    "precipitation_noaa": "noaa_precipitation",
    "temperature_consumption_forecast_volue": "volue_temperature_consumption_forecast",
}
...
for source_table, target_table in table_mapping.items():
    src_table = f"{catalog_prefix}silver.{schema_prefix}{source_schema}.{source_table}"
    target_table = f"{catalog_prefix}silver.{schema_prefix}{target_schema}.{target_table}"
    print(f"Processing table: {src_table} -> {target_table}")
    if spark.catalog.tableExists(src_table):
        columns = spark.catalog.listColumns(src_table)
        partition_columns = [col.name for col in columns if col.isPartition]
        cluster_columns = [col.name for col in columns if col.isCluster]
        partition_clause = f"PARTITIONED BY ({', '.join(partition_columns)})" if partition_columns else ""
        cluster_clause = f"CLUSTER BY ({', '.join(cluster_columns)})" if cluster_columns else ""

        ddl = get_table_ddl_script(f"{catalog_prefix}silver", f"{schema_prefix}{source_schema}", source_table)
        tbl_comment = get_table_comment(src_table)
        query = f"""CREATE TABLE IF NOT EXISTS {target_table} (
                {ddl}
            ) {partition_clause} {tbl_comment}
            TBLPROPERTIES ('delta.feature.allowColumnDefaults' = 'supported') {cluster_clause} """
        spark.sql(f"ALTER TABLE {src_table} DROP CONSTRAINT IF EXISTS pk_{source_table}")
        spark.sql(query)
        (
            spark.read.table(f"{src_table}")
            .drop("table_id")
            .write.format("delta")
            .mode("append")
        ...
        spark.sql(f"DROP TABLE {src_table}")
