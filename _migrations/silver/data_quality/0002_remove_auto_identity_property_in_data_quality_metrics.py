...
full_table_name = f"{catalog_prefix}silver.{schema_prefix}{schema}.{table}"
dq_rules_schema = (
...
query_to_populate = f"""(
    select
        ...
        updated_by,
        CAST(
            CAST(ABS(HASH(CONCAT_WS('||', schema, table, name, evaluation, UUID()))) AS BIGINT) * 1000000 +
            CAST(RAND() * 999998 + 1 AS BIGINT) AS BIGINT
        ) AS table_id
